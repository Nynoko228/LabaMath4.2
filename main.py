...
def formula(n):
    ...
    match n:
        case 1:
            formula = "Формула для чётного количества интервалов: " r"$I=\int_{a}^{b}f(x)dx\approx \frac{h}{3}(y_{0}+4y_{1}+2y_{2}+4y_{3}+...+4y_{n-1}+y_{n})$ "
        case 2:
            formula = "Формула для нечётного количества интервалов: Для первых 4 значений y используем: " + r"$A_{3}=\frac{3h}{8}(y_{0}+3y_{1}+3y_{2}+y_{3})$ Для оставшихся используем: $\frac{h}{3}(4y_{4}+2y_{5}+4y_{6}+...+4y_{n-1}+y_{n})$"
    root = tk.Toplevel()
    app = Application(formula, master=root)
    app.mainloop()
